...
import { AnchorProps, message } from 'antd'
...
type PreCodeProps = PropsWithChildren<{}>
const PreCode = (props: PreCodeProps) => {
  const ref = useRef<HTMLPreElement>(null)
  const refText = ref.current?.innerText
  const langRef = useRef('')
  const renderMermaid = useDebouncedCallback(() => {
    if (!ref.current) return
    const { className } = ref.current.querySelector('code') as HTMLElement
...
interface componentsA extends ComponentProps<'a'> {}
  ...
  <section className={styles.markdownBody}>
    ...
    {markdownText && (
      <ReactMarkdown
        ...
        components={{
          pre: PreCode,
          p: (pProps: PropsWithChildren) => <p {...pProps} dir='auto' />,
          a: (aProps: componentsA) => {
